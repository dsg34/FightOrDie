...
Protagonista::Protagonista(sf::Sprite* s, sf::Texture* t, sf::Vector2<float> p, int mV, float ve) :Personaje(s,t,p,mV,ve) {
    ...
    texPistola = new sf::Texture(*t);
    
    sf::Texture texEsco;
    if (!texEsco.loadFromFile("resources/BillyEscopeta.png"))
    {
        std::cerr << "Error cargando la imagen sprites.png";
        exit(0);
    }
    texEscopeta = new sf::Texture(texEsco);

    sf::Texture texMetra;
    if (!texMetra.loadFromFile("resources/BillyAK47.png"))
    {
        std::cerr << "Error cargando la imagen sprites.png";
        exit(0);
    }
...
void Protagonista::sacarDeInventario(int posicion){
    inventario.erase(inventario.begin()+posicion);
}     
std::string Protagonista::intAString(int p){
    std::string devuelve;
...
void Protagonista::dispararSecundaria(sf::Vector2<int> posicionCursor){
    if(municionSecundaria>0)
    {
        ...
        bool eliminado = false;
        for(int i=0;i<inventario.size();i++){
            if(inventario[i]->getTipo()==7 && eliminado == false){
                inventario[i]->setReducirEnInventario(true);
                eliminado = true;                
            }     
        }    
...
void Protagonista::anyadirGranada(sf::Vector2<float> p){
    arma->anyadirGranada(p);
}

void Protagonista::recibirRecurso(Recurso* r){
    // Escopeta=2; Botiquin=3; Metralleta=4; Barril=5; Madera=6; Granada=7; Valla=8;
    if(r->getBloqueante()==0 || (r->getBloqueante()==1 && r->getTipo()==5)){
        int tipo = r->getTipo();
...
int Protagonista::Colision(std::vector<Zombie*> zombies, char direccion){
    ...
    sf::FloatRect* cajaZ;
    float posPX=spriteCopia->getPosition().x;
    float posPY=spriteCopia->getPosition().y;
    float posZX=0;
    float posZY=0;
    ...
    for(int i=0; i<zombies.size(); i++){
       ...
       {       
            posZX=zombies[i]->getSprite()->getPosition().x;
            posZY=zombies[i]->getSprite()->getPosition().y;
            if(direccion == 'D'){
                if((posPX < posZX) && (posPY+(75/2) > posZY) && (posPY-(75/2) < posZY))
                    choca=true;
            }else if(direccion == 'S'){
                if((posPY < posZY) && (posPX+(75/2) > posZX) && (posPX-(75/2) < posZX))
                    choca=true;
            }else if(direccion == 'A'){
                if((posPX > posZX) && (posPY+(75/2) > posZY) && (posPY-(75/2) < posZY))
                    choca=true;
...
void Protagonista::colisionConRecursos(std::vector<Recurso*> &recursos)
{
    ...
    //Recurso* r;
        ...
        {
            if(recursos[i]->getBloqueante()==0 && boundingBox->intersects(*recursos[i]->getBoundingBox()))
            {
                recibirRecurso(recursos[i]);
                recursos[i]->setExiste(false);
                //recursos.erase(recursos.begin()+i);                
            }else if(recursos[i]->getBloqueante()==1 && boundingBox->intersects(*recursos[i]->getBoundingBox()) &&  (sf::Joystick::isButtonPressed(mando, 6) || sf::Keyboard::isKeyPressed(sf::Keyboard::F))){
                recibirRecurso(recursos[i]);
                recursos[i]->setExiste(false);
                //recursos.erase(recursos.begin()+i);  
            }
...
bool Protagonista::devuelveSprintDisponible(){
    bool devuelve=false;          
    if(correr==false && relojSprint.getElapsedTime().asSeconds()>10){
        devuelve=true;
    }
    return devuelve;
}
...
int Protagonista::update(sf::Vector2<int> pos, std::vector<Zombie*> enemigos, MapLoader* m,std::vector<Recurso*> recursos, int &disp){
    ...
    *boundingBox = sprite->getGlobalBounds();
    ...
    if(vida > 0)
    {
        ...
        if(sf::Keyboard::isKeyPressed(sf::Keyboard::LControl) || (sf::Joystick::isButtonPressed(mando,0))){
            frecuencia = relojSprint.getElapsedTime();            
            if(correr==false && frecuencia.asSeconds()>10){
                velocidad=velocidad*2;
                correr=true;
                relojSprint.restart();
            }
